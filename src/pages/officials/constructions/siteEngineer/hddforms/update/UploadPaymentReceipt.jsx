...
const UploadPaymentReceipt = ({user,docId}) => {
    ...
    const handleUpload = () => {
      setIsLoading(true);
      if(!docId){
        return alert("document is not found to update.");
      }
      const url = `${serverUrl}/api/official-users/construction/site-engineers/hdd-form/update-payment-receipt`;
      ...
      if (screenshots && screenshots.length > 0) {
        screenshots.forEach((file) => {
          formData.append("paymentReceipt", file); // 'ss' is the field name for the file
        });
      }
    ...
    return (
      ...
      <div className="flex flex-col items-center">
        ...
        >
          Allowed file type (jpg, jpeg, png only)
        </label>
        <input
          type="file"
          id="ss"
